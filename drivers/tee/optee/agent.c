...
static struct tee_agent_kernel_ops *tee_kernel_agent_from_id
				(struct tee_context *ctx, unsigned int agent_id)
{
	int i;

	mutex_lock(&ctx->agent.mutex);
	for (i = 0; i < MAX_AGENT_NUM; i++)
		if (ctx->agent.tee_agent_ops[i] &&
		   (agent_id == ctx->agent.tee_agent_ops[i]->agent_id))
			break;

	if (i == MAX_AGENT_NUM) {
		mutex_unlock(&ctx->agent.mutex);
		return NULL;
	}
	mutex_unlock(&ctx->agent.mutex);

	return ctx->agent.tee_agent_ops[i];
}

int tee_agent_kernel_register(struct tee_context *ctx, struct tee_agent_kernel_ops *tee_ops)
{
	int i;
	...
	mutex_lock(&tee_context->agent.mutex);
	for (i = 0; i < MAX_AGENT_NUM; i++) {
		if (!tee_context->agent.tee_agent_ops[i])
			break;
	}
	...
	if (i == MAX_AGENT_NUM) {
		pr_err("Too many agent, the max num agent supported is \
							128!\n");
		mutex_unlock(&tee_context->agent.mutex);
		return -ENOMEM;
	}

	tee_context->agent.tee_agent_ops[i] = tee_ops;

	mutex_unlock(&tee_context->agent.mutex);

	tee_kernel_agent_set_available(tee_context->agent.tee_agent_ops[i], true);

	return 0;
}
EXPORT_SYMBOL(tee_agent_kernel_register);

int tee_agent_kernel_unregister(struct tee_context *ctx, struct tee_agent_kernel_ops *tee_ops)
{
	int i;
	...
	for (i = 0; i < MAX_AGENT_NUM; i++)
		if (ctx->agent.tee_agent_ops[i] &&
		   (tee_ops->agent_id == ctx->agent.tee_agent_ops[i]->agent_id))
			break;
	...
	ctx->agent.tee_agent_ops[i] = NULL;
...
int optee_tee_agent_cmd(struct tee_context *ctx, unsigned int agent_id, void *virt, unsigned int size)
{
	int i, rc;
	...
	mutex_lock(&ctx->agent.tee_agent_ops[i]->mutex);

	if (!ctx->agent.tee_agent_ops[i]->avail) {
		mutex_unlock(&ctx->agent.tee_agent_ops[i]->mutex);
		return -ENXIO;
	}
	rc = ctx->agent.tee_agent_ops[i]->tee_agent_run(ctx->agent.tee_agent_ops[i]->agent_data,
					      virt, size);
...
int optee_agent_register(struct tee_context *ctx, u32 agent_id)
{
	...
	rc = tee_agent_kernel_register(ctx, &agent->ops);
...
int optee_agent_unregister(struct tee_context *ctx, u32 agent_id)
{
	...
	kagent = tee_kernel_agent_from_id(ctx, agent_id);
	if (!kagent)
		return -ENXIO;
	...
	if (tee_agent_kernel_unregister(ctx, &agent->ops))
...
void optee_agent_release(struct tee_context *ctx)
{
	...
	for (i = 0; i < MAX_AGENT_NUM, i++) {
        if (ctx->agent.tee_agent_ops[i])
		    optee_agent_unregister(ctx, ctx->agent.tee_agent_ops[i].agent_id);
	}
}
