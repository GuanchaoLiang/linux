...
static void handle_rpc_func_cmd_agent(struct tee_context *ctx, struct optee_msg_arg *arg)
{
	...
	ret = optee_tee_agent_cmd(ctx, cmd, shm_va, shm_size);
	if (ret)
		goto bad;
				...
				struct optee_call_ctx *call_ctx)
{
	...
	case OPTEE_MSG_RPC_CMD_AGENT:
		handle_rpc_func_cmd_agent(ctx, arg);
		break;
